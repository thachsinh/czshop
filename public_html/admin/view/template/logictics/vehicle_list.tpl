        ...
        <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form-vehicle">
          <div class="table-responsive">
            <table class="table table-bordered table-hover">
              <thead>
                <tr>
                  <td style="width: 1px;" class="text-center"><input type="checkbox" onclick="$('input[vin*=\'selected\']').prop('checked', this.checked);" /></td>
                  <td class="text-left"><?php if ($sort == 'vin') { ?>
              ...
              <tbody>
                ...
                <tr>
                  <td class="text-center"><?php if (in_array($vehicle['vehicle_id'], $selected)) { ?>
                    ...
                    <?php } ?></td>
                  <td class="text-left"><a href="<?php echo $vehicle['edit']; ?>" data-toggle="tooltip" title="<?php echo $button_edit; ?>"><?php echo $vehicle['vin']; ?></a></td>
                  <td class="text-left"><?php echo $vehicle['model']; ?></td>
                  <td class="text-right">
                    <input type="checkbox" <?php echo ((isset($vehicle['status']) && ($vehicle['status'] == 1)) ? 'checked': ''); ?> data-toggle="toggle" data-size="small" data-cid="<?php echo $vehicle['vehicle_id']; ?>" class="btn-status" />
                  </td>
                  <td class="text-right">
                    <div class="btn-group">
                      ...
                      <a data-cid="<?php echo $vehicle['vehicle_id']; ?>" data-toggle="tooltip" title="<?php echo $button_delete; ?>" class="btn btn-danger"><i class="fa fa-trash-o"></i></a>
                    </div>
                  </td>
                </tr>
                <?php } ?>
...
<script type="text/javascript">
  ...
  $('input.btn-status').change(function() {
    ...
    $.post('<?php echo $ajax_status; ?>',
            {'vehicle_id': cid, 'status': status}, function(){} );
  })
</script>