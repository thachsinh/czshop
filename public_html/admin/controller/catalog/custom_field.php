<?php
class ControllerCatalogCustomField extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/custom_field');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_custom_field->addCustomField($this->request->post);
			...
			$this->response->redirect($this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_custom_field->editCustomField($this->request->get['custom_field_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $custom_field_id) {
				$this->model_catalog_custom_field->deleteCustomField($custom_field_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('catalog/custom_field/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/custom_field/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['ajax_delete'] = $this->url->linkajax('catalog/custom_field/ajaxdelete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['ajax_status'] = $this->url->linkajax('catalog/custom_field/ajaxstatus', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$custom_field_total = $this->model_catalog_custom_field->getTotalCustomFields();

		$results = $this->model_catalog_custom_field->getCustomFields($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['custom_fields'][] = array(
				...
				'edit'            => $this->url->link('catalog/custom_field/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $result['custom_field_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . '&sort=cfd.name' . $url, 'SSL');
		$data['sort_location'] = $this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.location' . $url, 'SSL');
		$data['sort_type'] = $this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.type' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.status' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/custom_field_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['custom_field_id'])) {
			$data['action'] = $this->url->link('catalog/custom_field/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/custom_field/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $this->request->get['custom_field_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('catalog/custom_field', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['custom_field_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$custom_field_info = $this->model_catalog_custom_field->getCustomField($this->request->get['custom_field_id']);
		}
		...
		if (isset($this->request->post['custom_field_description'])) {
			...
			$data['custom_field_description'] = $this->model_catalog_custom_field->getCustomFieldDescriptions($this->request->get['custom_field_id']);
		} else {
		...
		if (isset($this->request->post['custom_field_value'])) {
			...
			$custom_field_values = $this->model_catalog_custom_field->getCustomFieldValueDescriptions($this->request->get['custom_field_id']);
		} else {
		...
		if (isset($this->request->post['custom_field_group'])) {
			$custom_field_groups = $this->request->post['custom_field_group'];
		} elseif (isset($this->request->get['custom_field_id'])) {
			$custom_field_groups = $this->model_catalog_custom_field->getCustomFieldGroup($this->request->get['custom_field_id']);
			//print_r($custom_field_groups); exit;
		} else {
			$custom_field_groups = array();
		}

		$data['custom_field_group_selected'] = array();

		foreach ($custom_field_groups as $custom_field_group) {
			$data['custom_field_group_selected'][] = $custom_field_group['custom_field_group_id'];
		}

		$data['custom_field_required'] = array();

		foreach ($custom_field_groups as $custom_field_group) {
			if ($custom_field_group['required']) {
				$data['custom_field_required'][] = $custom_field_group['custom_field_group_id'];
			}
		}

		$this->load->model('catalog/custom_field_group');

		$data['custom_field_groups'] = $this->model_catalog_custom_field_group->getCustomFieldGroups();

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('catalog/custom_field_form.tpl', $data));
	}
	...
	public function ajaxStatus() {
		...
		if($custom_field_id > 0) {
			$this->load->model('catalog/custom_field');
			$this->model_catalog_custom_field->editStatus($custom_field_id, $status);
		}
	}
}