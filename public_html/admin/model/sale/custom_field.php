...
class ModelSaleCustomField extends Model {
	...
	public $fields = array('custom_field_id', 'type', 'value', 'location', 'status', 'sort_order');
	...
	public function addCustomField($data) {
		...
		if (isset($data['custom_field_customer_group'])) {
			foreach ($data['custom_field_customer_group'] as $custom_field_customer_group) {
				if (isset($custom_field_customer_group['customer_group_id'])) {
					...
					$this->db->set('customer_group_id', (int)$custom_field_customer_group['customer_group_id']);
					$this->db->set('required', (int)(isset($custom_field_customer_group['required']) ? 1 : 0));
					$this->db->insert('custom_field_customer_group');
		...
		if (isset($data['custom_field_value'])) {
			foreach ($data['custom_field_value'] as $custom_field_value) {
				...
				$this->db->insert('custom_field_value');
	...
	public function editCustomField($custom_field_id, $data) {
		...
		$this->db->where($this->primaryKey, (int)$custom_field_id);
		$this->db->delete('custom_field_customer_group');
		...
		$this->db->where($this->primaryKey, (int)$custom_field_id);
		$this->db->delete(array('custom_field_value', 'custom_field_value_description'));
	...
	public function deleteCustomField($custom_field_id) {
		$this->db->where($this->primaryKey, (int)$custom_field_id);
		$this->db->delete(array($this->table, 'custom_field_description', 'custom_field_customer_group', 'custom_field_value', 'custom_field_value_description'));
		/*
	...
	public function getCustomField($custom_field_id) {
		...
		$this->db->join('custom_field_description cfd', 'cf.custom_field_id = cfd.custom_field_id', 'left');
	...
	public function getCustomFields($data = array()) {
		if (empty($data['filter_customer_group_id'])) {
			...
			$this->db->select('*');
			$this->db->from('custom_field_customer_group cfcg');
			$this->db->join('custom_field cf', 'cfcg.custom_field_id = cf.custom_field_id', 'left');
			$this->db->join('custom_field_description cfd', 'cf.custom_field_id = cfd.custom_field_id', 'left');
			$this->db->where('cfd.language_id', (int)$this->config->get('config_language_id'));
		...
		if (!empty($data['filter_customer_group_id'])) {
			$this->db->where('cfcg.customer_group_id', (int)$data['filter_customer_group_id']);
	...
	public function getCustomFieldDescriptions($custom_field_id) {
		...
		$this->db->select('*');
		$this->db->from('custom_field_description');
		$this->db->where($this->primaryKey, (int)$custom_field_id);
	...
	public function getCustomFieldCustomerGroups($custom_field_id) {
		$this->db->select('*');
		$this->db->from('custom_field_customer_group');
		$this->db->where($this->primaryKey, (int)$custom_field_id);
		return $this->db->get()->result_array();
	...
	public function getCustomFieldValueDescriptions($custom_field_id) {
		...
		$custom_field_value_query = $this->db->select('*')->from('custom_field_value')->where($this->primaryKey, (int)$custom_field_id)->get()->result_array();
		...
		foreach ($custom_field_value_query as $custom_field_value) {
			...
			$custom_field_value_description_query = $this->db->select('*')->from('custom_field_value_description')->where('custom_field_value_id', (int)$custom_field_value['custom_field_value_id'])->get()->result_array();
