...
class ModelAccountReturn extends Model {
	public $table = 'return';
	public $fields = array('order_id', 'customer_id', 'firstname', 'lastname', 'email', 'telephone', 'product', 'model', 'quantity', 'opened', 'return_reason_id', 'comment', 'date_ordered', 'date_added', 'date_modified');

	public function addReturn($data) {
		$this->event->trigger('pre.return.add', $data);

		$tmp = $this->initData($data, true);
		$tmp['customer_id'] = (int)$this->customer->getId();
	...
	public function getReturn($return_id) {
		...
		foreach($query as $item) {
			$returnStatus[$item['return_status_id']] = $item['name'];
		}

		$this->db->select('return_reason_id, name')
			->from('return_reason')
			->where('rs.language_id', (int)$this->config->get('config_language_id'));
		$query = $this->db->get()->result_array();
		$returnReason = array();
		foreach($query as $item) {
			$returnReason[$item['return_reason_id']] = $item['name'];
		}

		$this->db->select('return_action_id, name')
			->from('return_action')
			->where('rs.language_id', (int)$this->config->get('config_language_id'));
		$query = $this->db->get()->result_array();
		$returnAction = array();
		foreach($query as $item) {
			$returnAction[$item['return_action_id']] = $item['name'];
		}
		...
		$return['reason'] = $returnReason[$return['return_reason_id']];
		$return['action'] = $returnReason[$return['return_action_id']];
		$return['status'] = $returnReason[$return['return_status_id']];
	...
	public function getReturns($start = 0, $limit = 20) {
		...
		$this->db->select('return_id, order_id, firstname, lastname, return_status_id, date_added')
			->from('return')
			->where('customer_id', (int)$this->customer->getId())
			->order_by('return_id', 'DESC')
			->limit($limit, $start);

		$returns = $this->db->get()->result_array();

		foreach($returns as &$item) {
			$item['status'] = $returnStatus[$item['return_status_id']];
		}
